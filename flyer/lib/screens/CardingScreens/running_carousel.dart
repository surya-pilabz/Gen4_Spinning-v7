            ...
            builder: (context, snapshot) {
              try {
                if (snapshot.hasData) {
                  ...
                  Map<String, String> _carouselResponse =
                      CarouselMessage(carouselId: _carousalID).decode(_d);
                  ...
                  if (!_carouselResponse.isEmpty) {
                    if (_carousalID == "0A") {
                      //for production
                      production =
                          double.parse(_carouselResponse["outputMtrs"]!)
                              .toStringAsFixed(2);
                      totalPower =
                          double.parse(_carouselResponse["totalPower"]!)
                              .toStringAsFixed(0);
                      //print ("after Carousal Response :$production , $totalPower");
                    } else {
                      motorTemp = double.parse(_carouselResponse["motorTemp"]!)
                          .toStringAsFixed(0);
                      MOSFETTemp =
                          double.parse(_carouselResponse["MOSFETTemp"]!)
                              .toStringAsFixed(0);
                      current = double.parse(_carouselResponse["current"]!)
                          .toStringAsFixed(2);
                      RPM = double.parse(_carouselResponse["RPM"]!)
                          .toStringAsFixed(0);
                    }
                  }
                  ...
                  return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        i != 1
                            ? _customRow("Motor Temp (C)", motorTemp ?? "-")
                            : _customRow("Total Length (m)", production ?? "-"),
                        i != 1
                            ? _customRow("MOSFET Temp (C)", MOSFETTemp ?? "-")
                            : _customRow("Total Power (W)", totalPower ?? "-"),
                        i != 1
                            ? _customRow("Current (A)", current ?? "-")
                            : Container(),
